...
export async function bulkInsertVotesFromExternalPolisConvo({
    ...
    async function doImportVotes(db: PostgresJsDatabase) {
        ...
        for (const insertedVote of insertVoteResponse) {
            ...
            if (voteContentToModify === undefined) {
                throw new Error(
                    `Data is out of sync while importing voteId=${String(insertedVote.voteId)}, polisVoteId=${String(insertedVote.polisVoteId)} with conversationSlugId=${conversationSlugId}`,
                );
            }
